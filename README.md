...
`localStorage` 是按浏览器和站点隔离的。Chrome、Edge、Firefox 之间不会自动共享数据。换浏览器前请先导出正式数据库 JSON 和候选池工作流 JSON。
